...
```bash
/feature-new "Transaction Filters" --milestone "v1.0"

# Creates:
#   docs/features/PM-001-transaction-filters/   (in this repo)
#   ├── spec.md
#   ├── plan.md
#
# GitHub (in financy-project/features, if gh is configured):
#   - Milestone: "v1.0" (if not exists)
#   - Issue: "PM-001: Transaction Filters" (linked to milestone)
#
# Git: branch PM-001/transaction-filters checked out (in this repo)
```

## Next Steps

1. Fill out `spec.md` with feature description and acceptance criteria (or collaborate with the `product-owner` agent)
