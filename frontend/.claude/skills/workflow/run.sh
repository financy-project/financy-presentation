...
# prompt cache for this entire block instead of the agent re-reading the DoR
# doc (and often exploring other components "to confirm") on every phase.
CHECKLIST_CONTENT=$(cat docs/architecture/dor.md)
STATIC_SYSTEM_PROMPT="$(cat .claude/agents/frontend-engineer.md)

---

# Definition of Ready / Blueprint Reference (already loaded — do NOT re-read this file, do NOT open docs/architecture/*.md, and do NOT grep other components for patterns unless something is genuinely ambiguous. This is the complete, current blueprint reference for every layer.)
...
while IFS= read -r line; do
  if [[ "$line" =~ $DETECT_PAT ]]; then
    ...
    BRANCH_PENDING=$(echo "$BRANCH_TASKS" | grep "^- \[ \] F-" || true)
...
while [ $ITERATION -lt $MAX_ITERATIONS ]; do
  ...
  if [ -n "$PHASE_BRANCH_EXISTS" ]; then
    ...
    PHASE_BRANCH_PENDING=$(echo "$PHASE_BRANCH_TASKS" | grep "^- \[ \] F-" || true)
...
## Blueprints (from phases/$PHASE_SLUG/blueprint.md — exact signatures, props, schemas; same content every phase)
\`\`\`
...
You are working on a frontend feature. Implement ALL pending tasks for Phase $CURRENT_PHASE_NUM: $CURRENT_PHASE_NAME.
...
## Instructions
The frontend engineer persona and the DoR/Blueprint reference are already
loaded in your system prompt. Do not re-read them, do not open
docs/architecture/*.md, and do not grep other components for patterns unless
something is genuinely ambiguous — follow the loaded reference directly.
...
4. Mark everything you just finished done — do NOT edit tasks.md/test-cases.md
   ...
   Call it once for the task's own F-NNN id, and once per T-NNN test case you
...
(one per line, e.g. \`useTransactionsQuery(filters: TransactionFilters): { transactions, isLoading, error }\`)
...
SETTINGS_EOF
  ...
  while IFS= read -r task_line; do
    TASK_ID=$(echo "$task_line" | grep -o 'F-[0-9]*' | head -1)
  ...
  COMPLETED=$(grep -c "^- \[x\] F-" "$TASKS_FILE" || echo 0)
