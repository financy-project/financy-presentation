...
#   ID is F-NNN (marks tasks.md + phases/*/tasks.md)
...
if [ -z "$PM_NUM" ] || [ -z "$ID" ]; then
  echo "Usage: mark-done.sh <PM-NNN> <F-NNN|T-NNN>" >&2
  exit 1
fi
...
case "$ID" in
  ...
  T-*)
    ...
    ;;
  *)
    echo "❌ Unrecognized id '$ID' — expected F-NNN (task) or T-NNN (test case)" >&2
    exit 1
    ;;
esac
