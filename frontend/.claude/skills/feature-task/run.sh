...
# "Test Cases" use this, per docs/architecture/dor.md's requirement that
...
write_generated_file() {
  ...
  if [ -z "$raw_content" ]; then
    printf '# %s %s\n\n_No content found — check if plan.md marks this section as Omitted (see docs/architecture/dor.md)._\n' "$PM_NUM" "$title" >"$out_file"
  else
...
TASKS_RAW=$(extract_section "Implementation Phases" "F")
write_generated_file "$TASKS_RAW" "Frontend Tasks" "$TASKS_FILE"

TOTAL_TASKS=$(grep -c "^- \[ \] [A-Z]-" "$TASKS_FILE" || true)
echo "✅ Generated $TASKS_FILE"
echo "   Total tasks: $TOTAL_TASKS"

# Soft granularity check (warns, never blocks): per docs/architecture/dor.md,
...
if [ "$SHALLOW_TASKS" -gt 0 ]; then
  ...
  echo "   Review the corresponding Implementation Phases bullets in $PLAN_FILE against docs/architecture/dor.md's granularity rule:"
...
if [ "$TOTAL_TEST_CASES" -gt 0 ]; then
  ...
  if [ "$SHALLOW_TEST_CASES" -gt 0 ]; then
    echo ""
    echo "⚠️  $SHALLOW_TEST_CASES/$TOTAL_TEST_CASES test case(s) have no backtick-quoted symbol — review against docs/architecture/dor.md's Test Cases blueprint."
  fi
fi
...
if [ -d "$PHASES_DIR" ]; then
  ...
  # Copy the whole Blueprints section (Component/GraphQL-API/Form &
  # Validation/State — exact signatures, props, schemas) into every phase
  # folder verbatim. Byte-identical across phases, so it hits Anthropic's
  # prompt cache from the second phase onward.
  BLUEPRINT_RAW=$(awk '
  ...
  if [ -n "$BLUEPRINT_RAW" ]; then
    ...
    echo "⚠️  No '## Definition of Ready (DoR) Blueprints' section found in $PLAN_FILE — phase folders have no blueprint.md. See docs/architecture/dor.md."
  fi
fi
